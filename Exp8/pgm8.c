...
int main() {
    ...
    if (E(E_rule, Edash_rule, T_rule, Tdash_rule, F_rule) && *cursor == '\0') { // If parsing is successful and the cursor has reached the end
        puts("--------------------------------");
...
int E(const char* E_rule, const char* Edash_rule, const char* T_rule, const char* Tdash_rule, const char* F_rule) {
    printf("%-16s %s\n", cursor, E_rule);
    if (T(T_rule, Tdash_rule, F_rule)) { // Call non-terminal T
...
int T(const char* T_rule, const char* Tdash_rule, const char* F_rule) {
    printf("%-16s %s\n", cursor, T_rule);
    if (F(F_rule)) { // Call non-terminal F
...
int F(const char* F_rule) {
    if (*cursor == '(') {
        printf("%-16s %s\n", cursor, F_rule);
        cursor++;
        if (E(F_rule, F_rule, F_rule, F_rule, F_rule)) { // Call non-terminal E
            if (*cursor == ')') {
                cursor++;
                return SUCCESS;
            } 
